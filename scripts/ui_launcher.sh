...
if [ -x /usr/sbin/iptables ]; then
    fw_ports="10081 5900 8765"
    # The doorbell video stream arrives on TCP :5000 only in the default
    # (TCP "warm") mode — i.e. when no RTP port is configured (video_rtp=0 or
    # unset in toonui.cfg). With an RTP port set, vpu_stream listens on that
    # UDP port instead (opened by toon1_video_prep), so :5000 isn't needed.
    vrtp=$(cfg_get video_rtp)
    if [ -z "${vrtp:-}" ] || [ "${vrtp:-0}" = "0" ]; then
        fw_ports="$fw_ports 5000"
    fi
    ...
    log "ensured firewall open for TCP: $fw_ports"
fi
...
# (backgrounded; survives the exec below) when installed — but ONLY if nothing
# is already running it. Some Toons supervise it from an inittab `respawn` row;
# starting a second copy would fight over the port, so we skip when it's up.
DOORBELL=/mnt/data/doorbell_daemon
if [ -x "$DOORBELL" ] && ! pidof doorbell_daemon >/dev/null 2>&1; then
