/* VENDORED gifdec.c — a copy of LVGL's GIF decoder kept in the main source
 * tree (out of the lvgl submodule) so our one patch survives a submodule
 * reset/re-checkout. The ONLY change vs upstream: gif_open() no longer bails
 * on a GIF with no Global Color Table — Buienradar's radar GIF has only
 * per-frame Local Color Tables, and read_image() loads those per frame.
 * The Makefile compiles this instead of lvgl/src/extra/libs/gif/gifdec.c.
 * Includes use the -Ilvgl path (this file lives a few dirs up from the orig). */
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    /* Presence of GCT. A GIF with no global color table (only per-frame local
     * color tables) is still valid — read_image() loads the LCT into the
     * palette per frame. Buienradar's radar GIF is exactly this case, so
     * don't bail: just treat the GCT as empty. */
    int has_gct = (fdsz & 0x80) != 0;
    /* Color Space's Depth */
    depth = ((fdsz >> 4) & 7) + 1;
    /* Ignore Sort Flag. */
    /* GCT Size (0 when absent) */
    ...
    gif->height = height;
    gif->depth  = depth;
    /* Read GCT (none when gct_sz == 0 — the LCT will fill the palette) */
    gif->gct.size = gct_sz;
    if (gct_sz) f_gif_read(gif, gif->gct.colors, 3 * gif->gct.size);
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
